...
//import com.employee.EmployeePortal.enums.*;
//import com.employee.EmployeePortal.exception.*;
...
//    @Autowired private PayslipItemRepository payslipItemRepository;
...
//    @Override
//    @Transactional
//    public PayslipDTO generatePayslip(String employeeId, LocalDate start, LocalDate end) {
//        Employee employee = employeeRepository.findById(employeeId)
//                .orElseThrow(() -> new ResourceNotFoundException("Employee not found: " + employeeId));
//
//        if (!payslipRepository.findByEmployeeIdAndPayPeriodBetween(employeeId, start, end).isEmpty())
//            throw new PayslipGenerationException("Payslip already exists for this period");
//
//        int workingDays = start.lengthOfMonth();
//        int presentDays = (int) attendanceRepository.findByEmployeeIdAndAttendanceDateBetween(employeeId, start, end)
//                .stream().filter(a -> "Present".equalsIgnoreCase(a.getStatus())).count();
//        BigDecimal ratio = BigDecimal.valueOf(presentDays).divide(BigDecimal.valueOf(workingDays), 2, RoundingMode.HALF_UP);
//
//        Payslip payslip = new Payslip();
//        payslip.setEmployee(employee);
//        payslip.setPayslipNumber("PSL-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
...
//        BigDecimal basic = BigDecimal.valueOf(5000).multiply(ratio);
//        BigDecimal hra = BigDecimal.valueOf(1000).multiply(ratio);
//        BigDecimal transport = BigDecimal.valueOf(300);
//
//        BigDecimal tax = basic.multiply(BigDecimal.valueOf(0.15));
//        BigDecimal ss = basic.multiply(BigDecimal.valueOf(0.05));
//
//        payslip.setBasicSalary(basic);
//        payslip.setHousingAllowance(hra);
//        payslip.setTransportAllowance(transport);
//        payslip.setTaxDeduction(tax);
//        payslip.setSocialSecurity(ss);
//
//        BigDecimal gross = basic.add(hra).add(transport);
//        BigDecimal deductions = tax.add(ss);
//        payslip.setGrossEarnings(gross);
//        payslip.setTotalDeductions(deductions);
//        payslip.setNetPay(gross.subtract(deductions));
//
//        payslipRepository.save(payslip);
//        createItem(payslip, LineItemType.EARNING, "Basic Salary", basic, true);
//        createItem(payslip, LineItemType.EARNING, "Housing Allowance", hra, true);
//        createItem(payslip, LineItemType.EARNING, "Transport Allowance", transport, false);
//        createItem(payslip, LineItemType.DEDUCTION, "Income Tax", tax, false);
//        createItem(payslip, LineItemType.DEDUCTION, "Social Security", ss, false);
//
//        return toDTO(payslip);
//    }
//
//    private void createItem(Payslip p, LineItemType type, String name, BigDecimal amt, boolean taxable) {
...
//        return toDTO(payslipRepository.findById(payslipId)
...
//        return ("Payslip PDF for " + payslipId).getBytes();
//    }
//
//    @Override
//    public void sendPayslipByEmail(Long payslipId, String recipientEmail) {
//        System.out.println("Emailing payslip " + payslipId + " to " + recipientEmail);
//    }
//
//    @Override
//    @Transactional
...
//        PayslipDTO dto = modelMapper.map(p, PayslipDTO.class);
//        dto.setEmployeeId(p.getEmployee().getEmployeeId());
//        dto.setEmployeeName(p.getEmployee().getFirstName() + " " + p.getEmployee().getLastName());
//        return dto;
//    }
//
//    private PayslipSummaryDTO toSummary(Payslip p) {
//        PayslipSummaryDTO s = new PayslipSummaryDTO();
//        s.setId(p.getId());
//        s.setPayslipNumber(p.getPayslipNumber());
//        s.setPayPeriodStart(p.getPayPeriodStart());
//        s.setPayPeriodEnd(p.getPayPeriodEnd());
//        s.setPayDate(p.getPayDate());
//        s.setStatus(p.getStatus().name());
//        s.setNetPay(p.getNetPay());
//        return s;
//    }
//}
